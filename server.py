    ...
    def _handle_animation(
        ...
        for player in status['players']:
            if player['id'] == command['id']:
                ...
                player['stats']['foreground_loc'] \
                    = command['stats']['foreground_loc']
                player['stats']['foreground_idx'] \
                    = command['stats']['foreground_idx']
    ...
    def _handle_speak(
        ...
        for player in status['players']:
            if player['id'] == command['id']:
                ...
                player['stats']['speaking_time'] \
                    = command['stats']['speaking_time']
        ...
        if 'to-enemy' in command['type']:
            for enemy in status['enemies']:
                if enemy['id'] == command['hitted']['id']:
                    ...
                    if not command['hitted']['stats']['alive']:
...
class PyMMOServer:
    def __init__(
        ...
        try:
            ...
            self.server.listen()
            self.server.settimeout(1.0)  # Allow timeout for KeyboardInterrupt
    ...
    def _create_player(
            self, address: str, time_connected: float) -> None:

        print(
            f'Connection has been established with: {address}'
            f' at {time_connected}. Welcome :)')
    ...
    def active_player_count(self) -> int:
        return threading.activeCount() - 1  # Exclude the main thread
